...
The Lean 4 formalization (\texttt{lean4/}) provides machine-verified guarantees with \textbf{zero \texttt{sorry} statements}---the proofs are complete with no deferred obligations. The verification includes 33+ theorems across four layers:
...
\midrule
Core Protocol & \texttt{TwoGenerals.lean} & 22 theorems \\
Network Model & \texttt{NetworkModel.lean} & 5 theorems \\
Timeout Logic & \texttt{TimeoutMechanism.lean} & 4 theorems \\
Main Results & \texttt{MainTheorem.lean} & 2 theorems \\
\bottomrule
\end{tabular}
\end{center}
...
The proofs are \textbf{self-contained}: they define their own axioms for cryptographic primitives (signatures, hashes) and time, without depending on external libraries like Mathlib. Build time: 5.5 seconds.
